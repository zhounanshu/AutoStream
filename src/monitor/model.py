...
class FeatureExtractor(nn.Module):
    ...
    def __init__(self):
        ...
        self.back_bone = nn.Sequential(*req_layers)
        for param in self.back_bone.named_parameters():
            param[1].requires_grad = True

    def forward(self, img):
        return self.back_bone(img)


class ProposalModel(nn.Module):
    def __init__(self, n_features, hid_dim=512, n_anchors=9, dropout=0.3):
        super(ProposalModel, self).__init__()
        self.n_anchors = n_anchors
        self.conv1 = nn.Conv2d(n_features, hid_dim, kernel_size=(3, 3), padding=1)
        self.dropout = nn.Dropout(dropout)
        self.conf_layer = nn.Conv2d(hid_dim,n_anchors, kernel_size=(1, 1))
        self.reg_layer = nn.Conv2d(hid_dim, n_anchors * 4, kernel_size=(1, 1))
    ...
    def forward(self, feature_map, pos_anc_ind=None, neg_anc_ind=None, pos_anc_coords=None):
        ...
        reg_offset_pred = self.reg_layer(out)
        conf_offset_pred = self.conf_layer(out)

        if mode == 'train':
            conf_score_pos = conf_offset_pred.flatten()[pos_anc_ind]
            conf_score_neg = conf_offset_pred.flatten()[neg_anc_ind]

            offset_pos = reg_offset_pred.contiguous().view(-1, 4)[pos_anc_ind]
            proposals = generate_proposals(pos_anc_coords, offset_pos)
            return conf_score_pos, conf_score_neg, offset_pos, proposals
        else:
            return conf_offset_pred, reg_offset_pred
...
class RegionProposalNetwork(nn.Module):
    ...
    def __init__(self, img_size, out_size, out_channels):
        ...
        self.w_scaler_factor = self.img_w // self.out_w
        self.h_scaler_factor = self.img_h // self.out_h
        ...
        self.proposal_module = ProposalModel(out_channels, self.n_anc_boxes)

    def forward(self, imgs, gt_bboxes, gt_cls):
        batch_size = imgs.size(dim=0)
        feature_map = self.feature_extractor(imgs)
        ...
        negative_anc_coords, positive_anc_ind_sep = get_req_anchors(anc_boxes_all, gt_bboxes_proj, gt_cls)
